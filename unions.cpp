...
void BENCH_NAME(shady::Runtime* runtime, shady::Device* device, shady::CompilerConfig* compiler_config) {
    ...
    shady::Buffer* buf_a = shady::allocate_buffer_device(device, a_size * sizeof(S));
    ...
    shady::copy_to_buffer(buf_a, 0, s, 16 * sizeof(S));
    uint64_t buf_a_addr = shady::get_buffer_device_pointer(buf_a);

    shady::Buffer* buf_b = shady::allocate_buffer_device(device, b_size * sizeof(int32_t));
    ...
    wait_completion(launch_kernel(program, device, "unions_kernel", b_size / 256, 1, 1, nargs, args, NULL));
    struct timespec ts;
    ...
    wait_completion(launch_kernel(program, device, "unions_kernel", b_size / 256, 1, 1, nargs, args, &extra_options));
    struct timespec tp;
    ...
    shady::copy_from_buffer(buf_b, 0, fin, b_size * sizeof(int32_t));
