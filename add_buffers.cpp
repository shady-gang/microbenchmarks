...
void BENCH_NAME(shady::Runner* runtime, shady::Device* device, shady::CompilerConfig* compiler_config) {
    ...
    shd_rn_wait_completion(shd_rn_launch_kernel(program, device, "add_buffers", buffer_size / 256, 1, 1, nargs, args, NULL));
    struct timespec ts;
    ...
    shd_rn_wait_completion(shd_rn_launch_kernel(program, device, "add_buffers", buffer_size / 256, 1, 1, nargs, args, &extra_options));
    struct timespec tp;
