...
void BENCH_NAME(shady::Runtime* runtime, shady::Device* device, shady::CompilerConfig* compiler_config) {
    ...
    wait_completion(launch_kernel(program, device, "fn_ptrs", buffer_size / 256, 1, 1, nargs, args, NULL));
    struct timespec ts;
    ...
    wait_completion(launch_kernel(program, device, "fn_ptrs", buffer_size / 256, 1, 1, nargs, args, &extra_options));
    struct timespec tp;
